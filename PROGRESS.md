...
- [x] **5.15 ปรับปรุง UI ทั่วทั้งแอป, อัปเดต App Icon ใหม่, แก้ไขแท็บตกขอบ และแปลภาษาครบ 100%**:
  - **เปลี่ยนไอคอนแอปพลิเคชันใหม่ (New App Icon)**:
    * ใช้รูปภาพโลโก้ "OURS - Job & Pealpeal" ที่ผู้ใช้มอบให้
    * อัปเดตไอคอน Android ทุกขนาด (`mdpi`, `hdpi`, `xhdpi`, `xxhdpi`, `xxxhdpi`) และเว็บไอคอน (`Icon-192.png`, `Icon-512.png`, `favicon.png`, etc.)
  - **แก้ไขปัญหาแท็บในหน้า Money ตกขอบ**:
    * ปรับชื่อแท็บที่ 3 ให้กระชับขึ้นเป็น `"Budget"` (ภาษาอังกฤษ) และ `"งบประมาณ"` (ภาษาไทย) จากเดิม "งบประมาณและโครงการ"
    * ตั้งค่า `tabAlignment: TabAlignment.fill` ทำให้ทั้ง 3 แท็บ (บัญชี / ธุรกรรม / งบประมาณ) แสดงผลพอดีกับหน้าจอมือถือ ไม่ล้นตกขอบและไม่ต้องเลื่อนจอ
  - **รองรับสองภาษาแบบไดนามิก (Full Bilingual Category & Screen Support)**:
    * สร้าง `CategoryNameHelper` (`c.localizedName(context)`) ให้หมวดหมู่รายรับ-รายจ่ายแสดงผลเป็นภาษาอังกฤษ (`nameEn`) เมื่อผู้ใช้เลือกภาษาอังกฤษ
    * แปลภาษาหน้าประวัติรายการธุรกรรม (`TransactionListScreen`), ตัวกรอง, กล่องค้นหา, ไดอะล็อกยืนยันการลบ
    * แปลภาษาหน้าจัดการหมวดหมู่ (`CategoriesScreen`) และไดอะล็อกเพิ่ม/แก้ไขหมวดหมู่ (`CategoryFormDialog`)
    * แปลภาษาหน้าตั้งงบประมาณ (`BudgetScreen`) และหน้ารายงานภาษี (`TaxScreen`)
    * แปลภาษาหน้าแผนการเงิน (`PlanScreen`) ครอบคลุมการ์ดเป้าหมาย, แถบสถานะ, ข้อแนะนำ
  - **ปรับแต่ง Layout & Contrast ไม่ให้ตัวอักษรหลุดกรอบทุกหน้าจอ**:
    * หน้ารายงานภาษี (`TaxScreen`): ย้ายตัวเลือกปีภาษีจาก AppBar ลงมาเป็นการ์ดในหน้าจอ ปรับแคปซูลสรุปยอดให้ใช้ `Wrap` ป้องกัน RenderFlex Overflow และปรับสีการ์ดปันผลให้คมชัดในโหมดมืด
    * หน้าสุขภาพการเงิน (`FinancialHealthScreen` & `LiabilitiesInsuranceScreen`): เปลี่ยนพื้นหลังกล่อง Run-rate forecast ให้ใช้ `Card` ของธีม แก้ปัญหาสีมืดกลืนตัวหนังสือ ปรับสถิติหนี้สินและประกันให้ใช้ `Wrap` ไม่ล้นกรอบ
  - **การรับประกันคุณภาพ**:
    * `flutter analyze`: **0 errors, 0 warnings**
    * `flutter test`: **116/116 ผ่านฉลุย 100%**
    * ตรวจสอบผลบนมือถือ Oppo Find X9 ผ่านหน้าจอจริง 100%
